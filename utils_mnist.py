...
mu = torch.tensor(mnist_mean).view(1, 1, 1).to(device)
...
def get_loaders_2(dir_, batch_size):
    train_transform = transforms.Compose([
...
def attack_fgsm(model, X, y, epsilon, alpha, attack_iters, restarts, opt=None):
    X_adv = X.detach().clone() # Since we don't want to modify x, we initialize x_adv as original benign image x
    X_adv.requires_grad = True # need to obtain gradient of x_adv, thus set required grad
    loss = F.cross_entropy(model(X_adv), y) # calculate loss
    loss.backward() # calculate gradient
    return epsilon * X_adv.grad.detach().sign()



def evaluate_pgd(test_loader, model, attack_iters, restarts, epsilon = (8 / 255.) / std):
    # alpha = torch.tensor((0.5, 0.5, 0.5)).view(3,  1, 1).to(device)
    alpha = torch.tensor(0.5).view(1,  1, 1).to(device)
    # alpha = (2/255.)/std
    pgd_loss = 0
    pgd_acc = 0
    n = 0
...
def evaluate_fgsm(test_loader, model, attack_iters, restarts, epsilon = (8 / 255.) / std):
    alpha = (2 / 255.) / std
    fgsm_loss = 0
    fgsm_acc = 0
    n = 0
    ...
    for i, (X, y) in enumerate(test_loader):
        X, y = X.cuda(), y.cuda()
        fgsm_delta = attack_fgsm(model, X, y, epsilon,
                               alpha, attack_iters, restarts)
        with torch.no_grad():
